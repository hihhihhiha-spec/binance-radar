...
def check_logic(symbol, tf):
    try:
        bars = exchange.fetch_ohlcv(symbol, timeframe=tf, limit=8)
        if len(bars) < 7: 
            return False
        ...
        for i in range(len(bars) - 3):
            ...
            # بيانات الشمعة الأولى (حمراء أولى)
            o1, h1, l1, cl1 = c1[1], c1[2], c1[3], c1[4]
            body1 = abs(o1 - cl1)
            
            # بيانات الشمعة الثانية (حمراء ثانية: أكبر من الأولى ولها ذيل سفلي)
            o2, h2, l2, cl2 = c2[1], c2[2], c2[3], c2[4]
            is_red_2 = cl2 < o2
            body2 = abs(o2 - cl2)
            lower_wick2 = min(o2, cl2) - l2
            
            # الشروط للشموع الهابطة: الحمراء الثانية أكبر من الأولى ولها ذيل سفلي واضح
            cond_red_setup = is_red_2 and (body2 > body1) and (lower_wick2 > 0)
            
            # بيانات الشمعة الثالثة (خضراء أولى ممتلئة داخل الشمعة الحمراء الثانية بالكامل)
            o3, h3, l3, cl3 = c3[1], c3[2], c3[3], c3[4]
            is_green_3 = cl3 > o3
            body3 = abs(o3 - cl3)
            is_green_full_3 = is_green_3 and (body3 > (h3 - l3) * 0.4) # ممتلئة وليست ديولاً فارغة
            is_inside_c2 = (h3 <= h2 and l3 >= l2) # بالكامل داخل الحمراء الثانية
            
            # بيانات الشمعة الرابعة (خضراء ثانية تكسر الأولى صعوداً، وبداخلها حمراء ممتلئة)
            # بما أننا نبحث عن الترتيب: حمراء 1 -> حمراء 2 (كبيرة وبداخلها خضراء 1) -> خضراء 2 (كاسرة وبداخلها حمراء)
            # لتبسيط وتطبيق الشروط الدقيقة المتفق عليها للنمط المتتالي:
            # نتحقق من الشمعة الرابعة والخامسة إذا لزم، أو نطبق النمط عبر الأربع شموع المتاحة:
            # c1: حمراء أولى
            # c2: حمراء ثانية (أكبر ولها ذيل سفلي)
            # c3: خضراء ممتلئة داخل c2 بالكامل
            # c4: خضراء تكسر c3 صعوداً (cl4 > cl3 أو h4 > h3) وبداخلها شمعة حمراء؟ 
            # انتظر، لنراجع ترتيبك بدقة:
            # 1. شمعتان نازلتان (الثانية أكبر من الأولى ولها ذيل لسفل).
            # 2. داخل الشمعة الحمراء الثانية شمعة خضراء ممتلئة.
            # 3. تليها شمعة خضراء تكسر الخضراء الأولى.
            # 4. الشمعة الخضراء الثانية تأتي بداخلها شمعة حمراء شرط أساسي (وكلها ديول وأجسام داخلها).
            
            # لنطبق هذا التسلسل بدقة على الشموع المتاحة (c1, c2, c3, c4):
            # c1: حمراء أولى
            # c2: حمراء ثانية (أكبر + ذيل سفلي)
            # c3: خضراء ممتلئة بالكامل داخل c2
            # c4: خضراء ثانية تكسر c3 صعوداً (h4 > h3 أو cl4 > cl3)
            # وبداخل c4 تأتي شمعة حمراء (أو نعتبر c4 هي الخضراء الكاسرة ونفحص ما بداخلها إذا توفرت شمعة c5، أو نكتفي بالمطابقة الحالية)
            
            # دعنا نأخذ 5 شموع للتأكد من اكتمال النموذج الخماسي بدقة مطلقة:
            pass

        # لضمان الدقة وتطبيق النمط الخماسي كاملاً (c1: حمراء، c2: حمراء أكبر + ذيل سفلي، c3: خضراء ممتلئة داخل c2، c4: خضراء كاسرة لـ c3، c5: حمراء داخل c4 بالكامل):
        if len(bars) < 5:
            return False
        ...
        for i in range(len(bars) - 4):
            ...
            # 1. شمعتان حمراوان (الثانية أكبر من الأولى ولها ذيل سفلي)
            o1, h1, l1, cl1 = c1[1], c1[2], c1[3], c1[4]
            body1 = abs(o1 - cl1)
            ...
            body2 = abs(o2 - cl2)
            lower_wick2 = min(o2, cl2) - l2
            cond_reds = is_red_2 and (body2 > body1) and (lower_wick2 > 0)
            
            # 2. داخل الشمعة الحمراء الثانية (c2) شمعة خضراء ممتلئة (c3) وذيولها بالكامل داخلها
            o3, h3, l3, cl3 = c3[1], c3[2], c3[3], c3[4]
            is_green_3 = cl3 > o3
            body3 = abs(o3 - cl3)
            ...
            # 3. تليها شمعة خضراء ثانية (c4) تكسر الشمعة الخضراء الأولى (c3) صعوداً
            o4, h4, l4, cl4 = c4[1], c4[2], c4[3], c4[4]
            is_green_4 = cl4 > o4
            is_c4_break = is_green_4 and (cl4 > cl3 or h4 > h3)
            
            # 4. والشمعة الخضراء الثانية (c4) تأتي بداخله شمعة حمراء (c5) وذيولها وأجسامها بالكامل بداخلها
            ...
            # التحقق من شروط النموذج الكاملة بدقة متناهية
...
send_telegram_message("🚀 تم تشغيل الرادار بالنمط الجديد والشرط الصارم للشموع مع طباعة الأخطاء بوضوح.")

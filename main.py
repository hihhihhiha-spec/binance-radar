...
class SimpleHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Parallel Multi-Strategy Radar (200 Coins) is Active")
...
# --- خيط الاستراتيجية الأولى ---
def run_strategy_1(get_symbols_func):
    ...
    while True:
        try:
            ...
            if not symbols:
                time.sleep(30)
                continue
            print(f"🚀 [خيط الاستراتيجية 1] البدء في الدورة رقم {cycle_counter}", flush=True)
            for idx, symbol in enumerate(symbols):
                for tf in timeframes:
                    print(f"🔍 [س1 | {idx+1}/{len(symbols)}] {symbol.upper()} | {tf}", flush=True)
...
# --- خيط الاستراتيجية الثانية ---
def run_strategy_2(get_symbols_func):
    timeframes = ['1m', '3m', '5m', '15m', '30m', '1h', '4h']
    cycle_counter = 1
    while True:
        try:
            ...
            if not symbols:
                time.sleep(30)
                continue
            print(f"🚀 [خيط الاستراتيجية 2] البدء في الدورة رقم {cycle_counter}", flush=True)
            for idx, symbol in enumerate(symbols):
                for tf in timeframes:
                    print(f"🔍 [س2 | {idx+1}/{len(symbols)}] {symbol.upper()} | {tf}", flush=True)
                    candles = get_klines(symbol, tf, limit=15)
                    if candles and len(candles) >= 7:
...
# --- خيط الاستراتيجية الثالثة الصارمة ---
def run_strategy_3(get_symbols_func):
    timeframes = ['1m', '3m', '5m', '15m', '30m', '1h', '4h']
    cycle_counter = 1
    while True:
        try:
            ...
            if not symbols:
                time.sleep(30)
                continue
            print(f"🚀 [خيط الاستراتيجية 3] البدء في الدورة رقم {cycle_counter}", flush=True)
            for idx, symbol in enumerate(symbols):
                for tf in timeframes:
                    print(f"🔍 [س3 | {idx+1}/{len(symbols)}] {symbol.upper()} | {tf}", flush=True)
                    candles = get_klines(symbol, tf, limit=15)
                    if candles and len(candles) >= 7:
                        ...
                        if cl1 < o1:
                            ...
                            if body1 > l_wick1 and l_wick1 > u_wick1:
                                ...
                                if cl2 < o2 and u_wick2 == 0 and l2 < l1 and cl2 < (l1 - l_wick1) and (body2 >= l_wick2 or l_wick2 >= body2):
                                    if cl3 > o3:
...
def main():
    print("🟢 [بدء التشغيل] تشغيل الرادار المتزامن (Parallel Radar) للاستراتيجيات الثلاث معاً...", flush=True)
    send_telegram_message("🟢 تم تشغيل الرادار المتزامن للاستراتيجيات الثلاث معاً لـ 200 عملة.")

    cached_symbols = []
    last_update_time = datetime.min

    def get_shared_symbols():
        nonlocal cached_symbols, last_update_time
        current_time = datetime.now()
        if not cached_symbols or (current_time - last_update_time >= timedelta(hours=3)):
            cached_symbols = get_top_futures_symbols(limit=200)
            last_update_time = current_time
        return cached_symbols

    # تشغيل كل استراتيجية في مسار مستقل (Thread) يعمل بالتزامن مع البقية
    t1 = threading.Thread(target=run_strategy_1, args=(get_shared_symbols,), daemon=True)
    t2 = threading.Thread(target=run_strategy_2, args=(get_shared_symbols,), daemon=True)
    t3 = threading.Thread(target=run_strategy_3, args=(get_shared_symbols,), daemon=True)

    t1.start()
    t2.start()
    t3.start()

    # إبقاء البرنامج الرئيسي حياً
    while True:
        time.sleep(60)

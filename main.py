...
# --- 2. إعدادات باينانس المباشرة المحمية ---
exchange = ccxt.binance({
    'options': {'defaultType': 'future'},
    'enableRateLimit': True,  # إدارة الطلبات تلقائياً لتجنب Ban
    'timeout': 20000
})
...
def get_active_symbols():
    """جلب أزواج العقود الآجلة النشطة"""
    try:
        markets = exchange.load_markets()
        symbols = [symbol for symbol, data in markets.items() if symbol.endswith('/USDT') and data.get('active', True)]
        print(f"✅ تم جلب {len(symbols)} عملة نشطة بنجاح من باينانس.", flush=True)
        return symbols
    except Exception as e:
        print(f"⚠️ يتعذر جلب قائمة العملات: {e}", flush=True)
        return []

# --- 3. خوارزمية النمط الثلاثي ---
    ...
    for index, symbol in enumerate(symbols, 1):
        ...
        for tf in TIMEFRAMES:
            try:
                ...
                # تأخير متزن لحماية الـ IP وحماية معدل الطلبات
                time.sleep(0.08)

            except ccxt.RateLimitExceeded:
                print("⚠️ تجاوز حد الطلبات! إيقاف مؤقت 20 ثانية...", flush=True)
                time.sleep(20)
            except Exception:
                continue

print("🚀 بدء تشغيل الرادار...", flush=True)
...
while True:
    try:
        if not symbols_list:
            ...
            if not symbols_list:
                print("⏳ إحالة وإعادة محاولة بعد 30 ثانية...", flush=True)
                time.sleep(30)
                continue
